...
%module(docstring="Hexrays Plugin SDK API wrapper",directors="1") hexrays
...
#ifdef __EA64__
  typedef unsigned long long int ea_t;      // effective address
#else
  typedef int ea_t;      // effective address
#endif
...
/*
    for obscure reasons swig can't get past this one on its own, it 
    needs a dummy declaration.
*/
class strvec_t {
public:
...
%rename(install_callback) __install_hexrays_callback;
%rename(remove_callback) __remove_hexrays_callback;
